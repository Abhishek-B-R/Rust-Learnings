...
# Now for final part, we put entire actual logic inside a loop and also did a typecasing, error handling.
...
## In rust, loops are called loop (as far as I know currently, let's see if this changes.)
## In rust we can **re-initialize** any variable with new data, here guess was already initialized as string, but we have changed it to i32.
## **match** is a control flow construct that allows you to match a value against a set of patterns and execute code based on the match.
## Here it is used 2 different ways, one for error handling and another for comparison.
## Later in comparison, the Ordering is an enum that represents the result of a comparison between two values.
